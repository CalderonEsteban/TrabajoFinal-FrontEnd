...
const fetchMoviesGrid = async (page = 1) => {
    ...
    const tendenciasContainer = document.querySelector('.sectionTendencias .galeriaTendencias');
    ...
    movies.forEach(movie => {
        ...
        const img = createElement('img', 'imagenPelicula', {
...
const fetchMoviesFlex = async () => {
    ...
    const aclamadasContainer = document.querySelector('.galeriaAclamadas');
    
    // Iteramos sobre cada película obtenida
    movies.forEach(movie => {
        // Creamos los elementos HTML para mostrar la película
        const peliculaItem = createElement('div', 'peliculaAclamada');
        const img = createElement('img', 'imagenAclamada', {
            src: `https://image.tmdb.org/t/p/w500/${movie.poster_path}`,
            alt: movie.title,
            loading: 'lazy'
        });
...
document.querySelector('.anterior').addEventListener('click', () => {
    ...
    let currentPage = Number(document.querySelector('.sectionTendencias').getAttribute('data-page'));
